...
class TicketList extends Component<any, IState>{
    ...
    private renderTickets(): JSX.Element[] {
        let appendElement = <CreateTicket key="new" projects={this.state.projects} handler={this.createHandle.bind(this)}/>
        let elements = this.state.tickets.slice().map(ticket => <Ticket onDelete={this.deleteHandle.bind(this)} ticket={ticket} key={ticket.id}/>)
